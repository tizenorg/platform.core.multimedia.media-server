...
/*		if (!_ms_dcm_check_process()) {
			MS_DBG_ERR("DCM server is not running!. Reset info for dcm server to execute");
...
gboolean _ms_dcm_agent_recv_dcm_done_from_server(GIOChannel *src, GIOCondition condition, gpointer data)
{
	...
	ms_dcm_server_msg recv_msg;

	MS_DBG_ERR("THUMB SERVER SOCKET %d", dcm_serv_sockfd);

	ret = ms_ipc_receive_message(dcm_serv_sockfd, &recv_msg, sizeof(ms_dcm_server_msg));
...
gboolean _ms_dcm_agent_send_msg_to_dcm_server(dcmMsg *recv_msg, dcmMsg *res_msg)
{
	...
	MS_DBG_SLOG("Sending msg to DCM service is successful");
	...
	if (res_msg->msg_type == 2 && g_dcm_comm_sock > 0) { // THUMB_REQUEST_ALL_MEDIA
		GSource *source = NULL;
...
gboolean _ms_dcm_agent_timer()
{
	...
	if (g_dcm_service_pid > 0) {
		...
		msg.msg_type = 4; // THUMB_REQUEST_KILL_SERVER
		msg.msg[0] = '\0';
		msg.msg_size = 0;
...
//		if (_ms_dcm_check_process() == FALSE) { // This logic is temporary
...
//		}
	...
	if (recv_msg) {
		if (recv_msg->msg_type == 1 && g_dcm_server_extracting) { // DCM_REQUEST_ALL_MEDIA
			...
			if (!_ms_dcm_agent_send_msg_to_dcm_server(recv_msg, &res_msg)) {
				...
				res_msg.msg_type = recv_msg->msg_type; // DCM_RESPONSE
