...
void ms_init_default_path(const char *storage_path){
	snprintf (default_path[0], MS_FILE_NAME_LEN_MAX, "%s/%s", storage_path, "Images");
	snprintf (default_path[1], MS_FILE_NAME_LEN_MAX, "%s/%s", storage_path, "Videos");
	snprintf (default_path[2], MS_FILE_NAME_LEN_MAX, "%s/%s", storage_path, "Sounds");
	snprintf (default_path[3], MS_FILE_NAME_LEN_MAX, "%s/%s", storage_path, "Downloads");
	snprintf (default_path[4], MS_FILE_NAME_LEN_MAX, "%s/%s", storage_path, "Camera");
}

void
ms_make_default_path_mmc(const char *storage_path)
{
	int i = 0;
	int ret = 0;
	DIR *dp = NULL;
 	ms_init_default_path(storage_path);
