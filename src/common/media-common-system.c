...
#define UID_DBUS_PATH		 "org/freedesktop/login1/Manager"
...
static int __ms_dbus_get_uid(const char *dest, const char *path, const char *interface, const char *method, uid_t *uid)
{
	...
	msg = dbus_message_new_method_call(dest, path, interface, method);
	if (!msg) {
		MS_DBG_ERR("dbus_message_new_method_call(%s:%s-%s)", path, interface, method);
		return MS_MEDIA_ERR_INTERNAL;
	}
	if (!dbus_message_get_args(msg, &err, DBUS_TYPE_UINT32, &value, DBUS_TYPE_INVALID)) {
		fprintf(stderr, "Wrong reply signature: %s", err.message);
		dbus_error_free(&err);
		result = -1;
		return result;
	}
...
int ms_sys_get_uid(uid_t *uid)
{
	...
	if (ret < 0) {
		MS_DBG_ERR("Failed to send dbus (%d)", ret);
	} else {
		MS_DBG_ERR("%d get UID[%d]", ret, *uid);
	}
