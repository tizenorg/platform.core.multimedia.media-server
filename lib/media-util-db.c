...
static int __media_db_connect_db_with_handle(sqlite3 **db_handle,uid_t uid)
{
	...
	if (access(mediadb_path, F_OK)) {
		if (MS_MEDIA_ERR_NONE == db_util_open_with_options(mediadb_path, db_handle, SQLITE_OPEN_READWRITE | SQLITE_OPEN_CREATE, NULL)) {
			...
			if ( uid == GLOBAL_USER){
				if(smack_setlabel(mediadb_path, "_", SMACK_LABEL_ACCESS)){
					MSAPI_DBG_ERR("failed chsmack -a \"_\" %s", mediadb_path);
				} else {
					MSAPI_DBG_ERR("chsmack -a \"_\" %s", mediadb_path);
				}
				asprintf(&mediadbjournal_path, "%s-journal", mediadb_path);
				if(smack_setlabel(mediadbjournal_path, "_", SMACK_LABEL_ACCESS)){
					MSAPI_DBG_ERR("failed chsmack -a \"_\" %s", mediadbjournal_path);
				} else {
					MSAPI_DBG_ERR("chsmack -a \"_\" %s", mediadbjournal_path);
				}
			} else {
...
int media_detach_view(MediaDBHandle *handle, uid_t uid){

	int ret = MS_MEDIA_ERR_NONE;
		
	if(uid != GLOBAL_USER) {
		char query_detach[MAX_MSG_SIZE];
		char query_drop_album[MAX_MSG_SIZE];
		char query_drop_folder[MAX_MSG_SIZE];
		char query_drop_playlist[MAX_MSG_SIZE];
		char query_drop_playlist_view[MAX_MSG_SIZE];
		char query_drop_tag_map[MAX_MSG_SIZE];
		char query_drop_bookmark[MAX_MSG_SIZE];
		char query_drop_media[MAX_MSG_SIZE];
		char query_drop_playlist_map[MAX_MSG_SIZE];
		char query_drop_tag[MAX_MSG_SIZE];
		char query_drop_tag_view[MAX_MSG_SIZE];
		
		snprintf(query_drop_album, MAX_MSG_SIZE, QUERY_DROP_VIEW_ALBUM);
		MSAPI_DBG_INFO("info : execute query_drop : %s", query_drop_album );
		ret = __media_db_update_directly(handle,query_drop_album);
		if ( ret != MS_MEDIA_ERR_NONE) {
			return ret;
		}

		snprintf(query_drop_folder, MAX_MSG_SIZE, QUERY_DROP_VIEW_FOLDER);
		MSAPI_DBG_INFO("info : execute query_drop : %s", query_drop_folder );
		ret = __media_db_update_directly(handle,query_drop_folder);
		if ( ret != MS_MEDIA_ERR_NONE) {
			return ret;
		}

		snprintf(query_drop_playlist, MAX_MSG_SIZE, QUERY_DROP_VIEW_PLAYLIST);
		MSAPI_DBG_INFO("info : execute query_drop : %s", query_drop_playlist );
		ret = __media_db_update_directly(handle,query_drop_playlist);
		if ( ret != MS_MEDIA_ERR_NONE) {
			return ret;
		}

		snprintf(query_drop_playlist_view, MAX_MSG_SIZE, QUERY_DROP_VIEW_PLAYLIST_VIEW);
		MSAPI_DBG_INFO("info : execute query_drop : %s", query_drop_playlist_view );
		ret = __media_db_update_directly(handle,query_drop_playlist_view);
		if ( ret != MS_MEDIA_ERR_NONE) {
			return ret;
		}

		snprintf(query_drop_tag_map, MAX_MSG_SIZE, QUERY_DROP_VIEW_TAG_MAP);
		MSAPI_DBG_INFO("info : execute query_drop : %s", query_drop_tag_map );
		ret = __media_db_update_directly(handle,query_drop_tag_map);
		if ( ret != MS_MEDIA_ERR_NONE) {
			return ret;
		}

		snprintf(query_drop_bookmark, MAX_MSG_SIZE, QUERY_DROP_VIEW_BOOKMARK);
		MSAPI_DBG_INFO("info : execute query_drop : %s", query_drop_bookmark );
		ret = __media_db_update_directly(handle,query_drop_bookmark);
		if ( ret != MS_MEDIA_ERR_NONE) {
			return ret;
		}

		snprintf(query_drop_media, MAX_MSG_SIZE, QUERY_DROP_VIEW_MEDIA);
		MSAPI_DBG_INFO("info : execute query_drop : %s", query_drop_media );
		ret = __media_db_update_directly(handle,query_drop_media);
		if ( ret != MS_MEDIA_ERR_NONE) {
			return ret;
		}

		snprintf(query_drop_playlist_map, MAX_MSG_SIZE, QUERY_DROP_VIEW_PLAYLIST_MAP);
		MSAPI_DBG_INFO("info : execute query_drop : %s", query_drop_playlist_map );
		ret = __media_db_update_directly(handle,query_drop_playlist_map);
		if ( ret != MS_MEDIA_ERR_NONE) {
			return ret;
		}
		
		snprintf(query_drop_tag, MAX_MSG_SIZE, QUERY_DROP_VIEW_TAG);
		MSAPI_DBG_INFO("info : execute query_drop : %s", query_drop_tag );
		ret = __media_db_update_directly(handle,query_drop_tag);
		if ( ret != MS_MEDIA_ERR_NONE) {
			return ret;
		}

		snprintf(query_drop_tag_view, MAX_MSG_SIZE, QUERY_DROP_VIEW_TAG_VIEW);
		MSAPI_DBG_INFO("info : execute query_drop : %s", query_drop_tag_view );
		ret = __media_db_update_directly(handle,query_drop_tag_view);
		if ( ret != MS_MEDIA_ERR_NONE) {
			return ret;
		}
		
		snprintf(query_detach, MAX_MSG_SIZE, QUERY_DETACH, __media_get_media_DB(GLOBAL_USER));
		MSAPI_DBG_INFO("info : execute query_detach : %s", query_detach );
		ret = __media_db_update_directly(handle,query_detach);
		if ( ret != MS_MEDIA_ERR_NONE) {
			return ret;
		}
		
	} else {
		MSAPI_DBG_INFO(" Global user no need delate tmp view");
	}
...
int media_attach_view(MediaDBHandle *handle, uid_t uid){

	int ret = MS_MEDIA_ERR_NONE;
	
	if(uid != GLOBAL_USER) {
		char query_attach[MAX_MSG_SIZE];
		char query_view_album[MAX_MSG_SIZE];
		char query_view_folder[MAX_MSG_SIZE];
		char query_view_playlist[MAX_MSG_SIZE];
		char query_view_playlist_view[MAX_MSG_SIZE];
		char query_view_tag_map[MAX_MSG_SIZE];
		char query_view_bookmark[MAX_MSG_SIZE];
		char query_view_media[MAX_MSG_SIZE];
		char query_view_playlist_map[MAX_MSG_SIZE];
		char query_view_tag[MAX_MSG_SIZE];
		char query_view_tag_view[MAX_MSG_SIZE];
		char query_check_database[MAX_MSG_SIZE];
		
		snprintf(query_attach, MAX_MSG_SIZE, QUERY_ATTACH, __media_get_media_DB(GLOBAL_USER));
		MSAPI_DBG_INFO("info : execute query_attach : %s", query_attach );
		ret = __media_db_update_directly(handle,query_attach);
		if ( ret != MS_MEDIA_ERR_NONE) {
			return ret;
		}
		
		snprintf(query_view_album, MAX_MSG_SIZE, QUERY_CREATE_VIEW_ALBUM);
		MSAPI_DBG_INFO("info : execute query_attach : %s", query_view_album );
		ret = __media_db_update_directly(handle,query_view_album);
		if ( ret != MS_MEDIA_ERR_NONE) {
			return ret;
		}

		snprintf(query_view_folder, MAX_MSG_SIZE, QUERY_CREATE_VIEW_FOLDER);
		MSAPI_DBG_INFO("info : execute query_attach : %s", query_view_folder );
		ret = __media_db_update_directly(handle,query_view_folder);
		if ( ret != MS_MEDIA_ERR_NONE) {
			return ret;
		}

		snprintf(query_view_playlist, MAX_MSG_SIZE, QUERY_CREATE_VIEW_PLAYLIST);
		MSAPI_DBG_INFO("info : execute query_attach : %s", query_view_playlist );
		ret = __media_db_update_directly(handle,query_view_playlist);
		if ( ret != MS_MEDIA_ERR_NONE) {
			return ret;
		}

		snprintf(query_view_playlist_view, MAX_MSG_SIZE, QUERY_CREATE_VIEW_PLAYLIST_VIEW);
		MSAPI_DBG_INFO("info : execute query_attach : %s", query_view_playlist_view );
		ret = __media_db_update_directly(handle,query_view_playlist_view);
		if ( ret != MS_MEDIA_ERR_NONE) {
			return ret;
		}

		snprintf(query_view_tag_map, MAX_MSG_SIZE, QUERY_CREATE_VIEW_TAG_MAP);
		MSAPI_DBG_INFO("info : execute query_attach : %s", query_view_tag_map );
		ret = __media_db_update_directly(handle,query_view_tag_map);
		if ( ret != MS_MEDIA_ERR_NONE) {
			return ret;
		}

		snprintf(query_view_bookmark, MAX_MSG_SIZE, QUERY_CREATE_VIEW_BOOKMARK);
		MSAPI_DBG_INFO("info : execute query_attach : %s", query_view_bookmark );
		ret = __media_db_update_directly(handle,query_view_bookmark);
		if ( ret != MS_MEDIA_ERR_NONE) {
			return ret;
		}

		snprintf(query_view_media, MAX_MSG_SIZE, QUERY_CREATE_VIEW_MEDIA);
		MSAPI_DBG_INFO("info : execute query_attach : %s", query_view_media );
		ret = __media_db_update_directly(handle,query_view_media);
		if ( ret != MS_MEDIA_ERR_NONE) {
			return ret;
		}

		snprintf(query_view_playlist_map, MAX_MSG_SIZE, QUERY_CREATE_VIEW_PLAYLIST_MAP);
		MSAPI_DBG_INFO("info : execute query_attach : %s", query_view_playlist_map );
		ret = __media_db_update_directly(handle,query_view_playlist_map);
		if ( ret != MS_MEDIA_ERR_NONE) {
			return ret;
		}
		
		snprintf(query_view_tag, MAX_MSG_SIZE, QUERY_CREATE_VIEW_TAG);
		MSAPI_DBG_INFO("info : execute query_attach : %s", query_view_tag );
		ret = __media_db_update_directly(handle,query_view_tag);
		if ( ret != MS_MEDIA_ERR_NONE) {
			return ret;
		}

		snprintf(query_view_tag_view, MAX_MSG_SIZE, QUERY_CREATE_VIEW_TAG_VIEW);
		MSAPI_DBG_INFO("info : execute query_attach : %s", query_view_tag_view );
		ret = __media_db_update_directly(handle,query_view_tag_view);
		if ( ret != MS_MEDIA_ERR_NONE) {
			return ret;
		}
		
	} else {
		MSAPI_DBG_INFO(" Global user no need to create tmp view");
	}
